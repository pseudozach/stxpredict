;; This is stxpredict - A prediction market on Stacks
;; Create a new market by sending description, resolver account, resolve type: auto or manual and resolve date/time.
;; Once resolve date/time has passed, resolver can call resolveMarket with the final result to trigger payouts.
;; Limitations: Only yes/no questions, static payment per vote, one account can join only one side.
(define-map marketDatabase {marketId: int} {question: (string-ascii 99), balance: uint, paypervote: uint, resolver: principal, yescount: uint, nocount: uint, resolveTime: uint, resolveType: (string-ascii 10), resolved: bool, result: bool})
...
(define-public (createMarket (question (string-ascii 99)) (paypervote uint) (resolver principal) (resolveTime uint) (resolveType (string-ascii 10)))
  (begin
    (map-set marketDatabase {marketId: (incrementMarketId)} {question: question, balance: u0, paypervote: paypervote, resolver: resolver, yescount: u0, nocount: u0, resolveTime: resolveTime, resolveType: resolveType, resolved: false, result: false})
...
(define-public (resolveMarket (marketId int) (result bool))
  (let (
    ...
    (resolver (default-to 'ST15RGYVK9ACFQWMFFA2TVASDVZH38B4VAV4WF6BJ (get resolver (map-get? marketDatabase {marketId: marketId}))))
    )
...
;; get market result - not really needed
